...
type Version={id:string;nome:string;tipo:string;referencia:string;ordem:number}; type MusicDetail={versoes:Version[];letraMarkdown:string|null;letraAviso:string|null}; type Option={id:string;titulo:string;artista:string;versoes:Version[]};
...
export class ExecucaoPageComponent {
  ...
  readonly lives=signal<Live[]>([]); readonly live=signal<Live|null>(null); readonly options=signal<Option[]>([]); readonly blocks=signal<{id:string;nome:string}[]>([]); readonly selectedMusic=signal(''); readonly selectedVersion=signal(''); readonly playingItem=signal<Item|null>(null); readonly playback=signal<MusicDetail|null>(null); readonly playbackVersion=signal<Version|null>(null); readonly message=signal(''); readonly busy=signal(false); private draggedId='';
  ...
  action(item:Item,action:'play'|'tocada'|'pulada'|'desfazer_tocada',version?:Version):void {if(action==='desfazer_tocada'&&!confirm('Confirmar reversão da música tocada? Esta correção será auditada.'))return;const id=this.live()?.id;if(!id)return;this.busy.set(true);this.api.post(`/api/lives/${id}/itens/${item.id}/execucao`,{acao:action,idempotencyKey:this.key(),confirmado:action==='desfazer_tocada',...(version?{referencia:version.referencia,tipo:version.tipo}:{}),...(action==='play'?{playEm:new Date().toISOString()}: {})},x=>x as {execucao:Live}).subscribe({next:x=>{this.busy.set(false);this.live.set(x.execucao);},error:e=>{this.busy.set(false);this.message.set((e as Error).message);}});}
  openPlayer(item:Item):void {this.playingItem.set(item);this.playback.set(null);this.playbackVersion.set(null);this.api.get(`/api/v1/musicas/${item.musica_id}`,x=>x as {musica:MusicDetail}).subscribe({next:x=>{this.playback.set(x.musica);this.playbackVersion.set(x.musica.versoes.find(v=>v.referencia===item.referencia_reproducao&&v.tipo===item.tipo_reproducao)??x.musica.versoes[0]??null);},error:e=>this.message.set((e as Error).message)});}
  setPlaybackVersion(id:string):void {this.playbackVersion.set(this.playback()?.versoes.find(v=>v.id===id)??null);}
  playSelected():void {const item=this.playingItem(),version=this.playbackVersion();if(!item||!version)return;if(version.tipo==='youtube'){this.action(item,'play',version);return;}const player=document.querySelector<HTMLMediaElement>('[data-testid="execution-player"]');if(!player)return;player.play().then(()=>this.action(item,'play',version)).catch(()=>this.message.set('Não foi possível iniciar a reprodução local.'));}
