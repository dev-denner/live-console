...
export class ExecucaoPageComponent {
  ...
  playSelected():void {const item=this.playingItem(),version=this.playbackVersion();if(!item||!version)return;this.action(item,'play',version);if(version.tipo!=='youtube'){const player=document.querySelector<HTMLMediaElement>('[data-testid="execution-player"]');if(player)player.play().catch(()=>this.message.set('Play registrado, mas a mídia local não iniciou.'));}}
