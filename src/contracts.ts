...
export const execucaoActionSchema=z.object({acao:z.enum(['play','tocada','pulada','pendente','desfazer_tocada']),idempotencyKey:z.string().uuid(),posicaoReal:z.number().int().positive().optional(),observacao:z.string().nullable().optional(),referencia:z.string().min(1).optional(),tipo:z.enum(['youtube','audio','video']).optional(),confirmado:z.boolean().optional(),playEm:z.string().datetime().optional()}).strict();
