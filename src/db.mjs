...
export function getMusic(db,id) { const result=music(db.prepare('SELECT * FROM musicas WHERE id=?').get(id)); if(result) result.fontes=sources(db,id); return result||null; }
...
export function updateMusic(db,id,input) {
...
export function getLive(db,id) { const live=db.prepare('SELECT * FROM lives WHERE id=?').get(id); return live?{...live,itens:liveItems(db,id)}:null; }
export function listLives(db) { return db.prepare('SELECT * FROM lives ORDER BY criada_em DESC').all().map(x=>({...x,itens:liveItems(db,x.id)})); }
export function createLive(db,input={}) { if(!input.titulo?.trim()) throw new Error('titulo é obrigatório'); const id=input.id||randomUUID(), at=stamp(); db.prepare('INSERT INTO lives(id,titulo,data,status,observacoes,criada_em,atualizada_em) VALUES(?,?,?,?,?,?,?)').run(id,input.titulo.trim(),input.data??null,input.status??'rascunho',input.observacoes??null,at,at); return id; }
