...
export function createApp({ database = openDatabase(defaultDatabase), storageRoot = defaultStorage }: { database?: DatabaseSync; storageRoot?: string } = {}): FastifyInstance {
  ...
  app.post('/api/live-drafts/:id/converter', async (request, reply) => { const draft=getLiveDraft(database,(request.params as {id:string}).id,storageRoot); if(!draft)return reply.code(404).send({error:'Rascunho não encontrado'}); try { reply.code(201); return {live:convertLiveDraft(database,draft)}; } catch(error) { return reply.code(409).send({error:(error as Error).message}); } });
  ...
  app.post('/api/execucoes/:id/itens/:itemId/bloco', async (request:any,reply:any) => { const blocoId=request.body?.blocoId ?? null; if(blocoId!==null&&typeof blocoId!=='string')throw badRequest('blocoId inválido'); try{return {item:moveLiveItemBlock(database,request.params.id,request.params.itemId,blocoId)};}catch(error){return reply.code(409).send({error:(error as Error).message});} });
