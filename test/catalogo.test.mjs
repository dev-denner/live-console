import test from 'node:test'; import assert from 'node:assert/strict'; import http from 'node:http'; import { mkdtempSync, readFileSync, mkdirSync, writeFileSync } from 'node:fs'; import { tmpdir } from 'node:os'; import { join } from 'node:path';
...
test('exportação reimporta e caminhos/URLs ficam literais',()=>{const {db}=fixture(),id=createMusic(db,song({letra:'C:\\legacy\\letra.md'}));addSource(db,id,{nome:'url',tipo:'youtube',referencia:'https://example.test/a?x=1&y=2'});const dump=exportCatalog(db),second=fixture();confirmImport(second.db,dump,{storageRoot:second.dir});assert.equal(listMusic(second.db)[0].fontes[0].referencia,'https://example.test/a?x=1&y=2');assert.equal(listMusic(second.db)[0].letra_caminho,null);db.close();second.db.close();});
test('importação valida letras, remove caminhos quebrados e reporta avisos',()=>{
  const {dir,db}=fixture(); mkdirSync(join(dir,'letras'),{recursive:true}); writeFileSync(join(dir,'letras','valida.md'),'letra válida');
  const payload={musicas:[{artista:'A',titulo:'Válida',letraCaminho:'letras/valida.md'},{artista:'A',titulo:'Ausente',letraCaminho:'letras/ausente.md'},{artista:'A',titulo:'Absoluta',letraCaminho:join(dir,'letras','valida.md')} ]};
  const preview=inspectImport(db,payload,{storageRoot:dir}); assert.equal(preview.letrasNaoEncontradas.length,1); assert.equal(preview.letrasNaoEncontradas[0].index,1); assert.equal(preview.avisos.length,1);
  const applied=confirmImport(db,payload,{storageRoot:dir}); assert.equal(applied.created,3); const rows=listMusic(db);
  assert.equal(rows.find(row=>row.titulo==='Válida').letra_caminho,'letras/valida.md'); assert.equal(rows.find(row=>row.titulo==='Absoluta').letra_caminho,'letras/valida.md'); assert.equal(rows.find(row=>row.titulo==='Ausente').letra_caminho,null); db.close();
});
