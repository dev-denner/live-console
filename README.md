...
O catálogo permite criar, editar, pesquisar, filtrar, importar/exportar JSON e manter referências de YouTube, áudio e vídeo. O formato oficial recebe um objeto com `musicas[]`; cada música pode trazer todos os campos editáveis e `versoes[]` (objetos completos ou strings abreviadas). A identidade da importação é `artista + titulo + musicaBase` (com `musicaBase=titulo` quando ausente); uma versão é identificada por `tipo + referencia` ou pelo seu `id`. Se uma música já existir, seus campos recebidos são atualizados e as versões são mescladas de forma aditiva: versões antigas nunca são apagadas quando o arquivo traz apenas versões novas. URLs e caminhos são tratados literalmente. A prévia não grava nada; a confirmação revalida, é atômica por padrão e aceita modo parcial explícito. `xEmLives` existente nunca é atualizado pela importação.
